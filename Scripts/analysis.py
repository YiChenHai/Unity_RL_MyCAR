#!/usr/bin/env python3
"""
高级蒸馏工具：特征分析、可视化、决策树分类模型

本工具用于将Unity训练数据（CSV格式）转换为决策树分类规则（C#代码）
数据格式：10维特征（sensor0-5, last_action_state, actual_vz, actual_vx, actual_omega）+ 1维目标（discrete_action 0-10）

使用方法：
1. 生成if规则库（自动优化，推荐，平衡准确率和代码大小）：
   python analysis.py --data <CSV文件路径> --rules [输出文件路径] --auto-optimize --target-accuracy 0.95 --max-code-lines 50000

2. 生成if规则库（限制代码大小，适合Unity）：
   python analysis.py --data <CSV文件路径> --rules [输出文件路径] --auto-optimize --target-accuracy 0.90 --max-code-lines 20000
...
6. 完整流程（推荐）：
   python analysis.py --data <CSV文件路径> --compare --rules DecisionTreeRules.cs --auto-optimize --target-accuracy 0.95 --max-code-lines 50000

参数说明：
  --auto-optimize: 自动优化参数以达到目标准确率（强烈推荐！会自动寻找最优参数，同时限制代码大小）
  --target-accuracy: 自动优化模式下的目标准确率（默认0.95，建议0.90-0.95）
  --target-r2: 兼容旧参数名，等同于 --target-accuracy
...
优化策略说明：
  - 自动优化会优先选择满足准确率要求且代码最小的参数组合
  - 如果无法同时满足准确率和代码大小要求，会优先满足准确率，然后尽量减小代码
  - 建议：准确率=0.90-0.95，代码行数<50000，这样既能保证性能又不会导致Unity崩溃

示例：
   # 自动优化，平衡准确率和代码大小（推荐）
   python analysis.py --data training_data.csv --rules if_rules.cs --auto-optimize --target-accuracy 0.95 --max-code-lines 50000
   
   # 限制代码大小，适合Unity（如果Unity崩溃，使用这个）
   python analysis.py --data training_data.csv --rules if_rules.cs --auto-optimize --target-accuracy 0.90 --max-code-lines 20000
...
# ========== 数据文件配置 ==========
# CSV数据文件路径（必需）
DATA_FILE_PATH = "D:/XiaoYiFei/Project/Unity/XYF_Car_Test/Data_Record/training_data_20260222_182202.csv"
...
GENERATE_RULES = "DecisionTreeRules.cs"  # 例如: "DecisionTreeRules.cs" 或 None

# ========== 自动优化配置（推荐使用） ==========
# 是否启用自动参数优化（True=自动寻找最优参数，False=使用手动参数）
USE_AUTO_OPTIMIZE = True

# 目标准确率（自动优化模式下，会尽量达到此准确率）
# 建议范围：0.90-0.95（越高精度越好，但代码量可能越大）
# 注意：如果设置为0.90，优化器可能在达到0.90后就停止搜索，导致准确率较低
# 建议设置为0.93-0.95以获得更高的精度
TARGET_ACCURACY = 0.95  # 离散动作分类的目标准确率

# 兼容旧配置（已废弃，保留用于向后兼容）
TARGET_R2_X = None  # 已废弃，使用 TARGET_ACCURACY
TARGET_R2_W = None  # 已废弃，使用 TARGET_ACCURACY
...
class AdvancedDistillation:
    def __init__(self, csv_path):
        ...
        # 根据 MyCar_Agent.cs 的格式：
        # 特征(10维): sensor0-5, last_action_state, actual_vz, actual_vx, actual_omega
        # 目标(1维): discrete_action (0-10: 急左转→急右转)
        ...
        # 检查是否有预期的列名（有表头的CSV）
        has_expected_header = all(col in self.data.columns for col in self.feature_names + ["discrete_action"])
        
        if has_expected_header:
            # 有表头：使用列名索引（更安全）
            X_data = self.data[self.feature_names]
            self.y = self.data["discrete_action"].values
        else:
            # 无表头或列名不匹配：使用位置索引
            # 检查第一行是否为数值（如果是，说明无表头）
            first_row_numeric = all(isinstance(val, (int, float)) or (isinstance(val, str) and self._is_number_string(val)) 
                                   for val in self.data.iloc[0, :11] if pd.notna(val))
            
            if first_row_numeric:
                # 无表头：直接使用位置索引（前10列为特征，第11列为目标）
                X_data = self.data.iloc[:, :10]
                self.y = self.data.iloc[:, 10].values
            else:
                # 有表头但列名不匹配：跳过第一行（表头），使用位置索引
                print("⚠️  警告: CSV表头列名不匹配，跳过第一行（表头）")
                X_data = self.data.iloc[1:, :10]
                self.y = self.data.iloc[1:, 10].values
        ...
        y_series = pd.Series(pd.to_numeric(self.y, errors="coerce"))
        
        # 移除对应的目标值中的NaN
        valid_mask = pd.notna(y_series)
        X_data = X_data[valid_mask]
        self.y = y_series[valid_mask].values
        
        # 转换为整数（离散动作索引 0-10）
        self.y = self.y.astype(np.int32)
        
        # 检查动作值范围
        if self.y.min() < 0 or self.y.max() > 10:
            print(f"⚠️  警告: 动作值超出范围 [0-10]，实际范围: [{self.y.min()}, {self.y.max()}]")
            # 裁剪到有效范围
            self.y = np.clip(self.y, 0, 10).astype(np.int32)
    ...
    def check_data_quality(self):
        ...
        action_names = ["急左转", "大左转", "左转", "小左转", "微左转", "直行", 
                       "微右转", "小右转", "右转", "大右转", "急右转"]
        
        for action, count in zip(unique_actions, counts):
            action_name = action_names[action] if 0 <= action <= 10 else f"未知({action})"
            percentage = count / len(self.y) * 100
            print(f"  动作{action:2d} ({action_name:6s}): {count:6d} 样本 ({percentage:5.2f}%)")
        ...
        if imbalance_ratio > 10:
            print(f"\n⚠️  警告: 动作分布不平衡（最大/最小比例: {imbalance_ratio:.1f}）")
            print(f"     建议: 增加少数动作的训练数据")
        
        # 检查传感器值分布
        sensor_cols = [f"sensor{i}" for i in range(6)]
        if all(col in self.data.columns for col in sensor_cols):
    ...
    def analyze_feature_importance(self, model_name="DecisionTree"):
        ...
        print("\n=== 特征重要性分析 ===")
        print("\n[离散动作分类 discrete_action]")
        sorted_idx = np.argsort(importance)[::-1]
        for idx in sorted_idx:
            print(f"  {self.feature_names[idx]:20s}: {importance[idx]:.4f}")
        
        # 绘制特征重要性
        fig, ax = plt.subplots(1, 1, figsize=(10, 6))
        
        top_idx = sorted_idx[:10] if len(sorted_idx) > 10 else sorted_idx
        ax.barh([self.feature_names[i] for i in top_idx], importance[top_idx])
        ax.set_xlabel("Importance")
        ax.set_title("Top Features for discrete_action Classification")
        ax.invert_yaxis()
    ...
    def compare_models(self, optimize_w=False, w_depth_boost=2):
        """对比不同深度的决策树性能"""
        depths = [3, 5, 7, 8, 10, 12, 15, 20, 25]
        results = {"depth": [], "accuracy": [], "nodes": []}
        
        print("\n=== 不同树深度的性能对比 ===")
        print(f"{'Depth':<8} {'Accuracy':<12} {'Nodes':<12} {'Code Est.'}")
        print("-" * 50)
        
        for depth in depths:
            tree = DecisionTreeClassifier(max_depth=depth, min_samples_leaf=5, min_samples_split=10, random_state=42)
            ...
            print(f"{depth:<8} {accuracy:<12.4f} {n_nodes:<12} ~{code_size} lines")
        
        # 绘制性能曲线
        fig, ax = plt.subplots(1, 1, figsize=(10, 6))
        
        ax.plot(results["depth"], results["accuracy"], marker='o', linewidth=2, label="Accuracy")
        ax.axhline(y=0.90, color='r', linestyle='--', alpha=0.5, label="0.90 threshold")
        ax.axhline(y=0.95, color='g', linestyle='--', alpha=0.5, label="0.95 threshold")
        ax.set_xlabel("Tree Depth")
        ax.set_ylabel("Accuracy")
        ax.set_title("Classification Accuracy vs Tree Depth")
        ax.legend()
        ...
        # 推荐深度
        print("\n[建议]")
        idx_best = np.argmax(results["accuracy"])
        best_depth = results["depth"][idx_best]
        best_accuracy = results["accuracy"][idx_best]
        print(f"  推荐深度: {best_depth} (准确率: {best_accuracy:.4f})")
    ...
    def find_optimal_params(self, target_accuracy=0.95, max_iterations=50, max_code_lines=50000):
        ...
        """
        print(f"\n=== 自动参数优化 ===")
        print(f"  目标准确率: {target_accuracy:.2f}")
        ...
        def optimize_tree(y_data, name, target_acc, max_code_lines_per_tree):
            """优化单个决策树，平衡准确率和代码大小"""
            best_params = None
            best_accuracy = 0.0
                ...
                for min_leaf in min_leaf_coarse:
                    for min_split in min_split_coarse:
                        ...
                        try:
                            ...
                            # 优先级：准确率优先，代码行数次要
                            # 如果准确率达到目标，在满足目标的方案中选择代码最小的
                            if accuracy >= target_acc:
                                # 达到目标准确率后，优先选择代码更小的（但不会为了代码更小而牺牲准确率）
                                if best_accuracy < target_acc or code_lines < best_code_lines:
                                    # 第一次达到目标，或者代码更小（准确率已经满足目标）
                                    ...
                                    print(f"  迭代 {iteration+1}: 准确率={accuracy:.4f}, 代码行数≈{code_lines} (depth={depth}, leaf={min_leaf}, split={min_split})")
        ...
        # 优化离散动作分类树
        params, accuracy = optimize_tree(self.y, "discrete_action", target_accuracy, max_code_lines)
        
        # 估算总代码行数
        tree_temp = DecisionTreeClassifier(
            max_depth=params['max_depth'] or 50,
            min_samples_leaf=params['min_samples_leaf'],
            min_samples_split=params['min_samples_split'],
            random_state=42
        )
        tree_temp.fit(self.X, self.y)
        total_code_lines = estimate_code_lines(tree_temp) + 50
        
        print(f"\n[优化结果]")
        print(f"  准确率: {accuracy:.4f}, 参数={params}, 代码行数≈{estimate_code_lines(tree_temp)}")
        print(f"  总代码行数估算: ≈{total_code_lines}")
        
        # 检查是否达到目标准确率
        if accuracy < target_accuracy:
            print(f"\n  ⚠️  未达到目标准确率 ({accuracy:.4f} < {target_accuracy:.2f})")
            print(f"     建议: 增加训练数据量或检查数据质量")
            print(f"     或者: 降低目标准确率到 {accuracy:.2f} 或更低")
        else:
            print(f"  ✓ 已达到目标准确率 ({accuracy:.4f} >= {target_accuracy:.2f})")
        
        if total_code_lines > max_code_lines:
            print(f"\n  ⚠️  警告: 总代码行数 ({total_code_lines}) 超过限制 ({max_code_lines})")
            print(f"     建议: 降低目标准确率或增加 --max-code-lines 参数")
    ...
    def generate_json_model(self, output_path="model.json", max_depth=8):
        ...
        model = {
            "metadata": {
                ...
                "num_classes": 11,
                "class_names": ["急左转", "大左转", "左转", "小左转", "微左转", "直行", 
                               "微右转", "小右转", "右转", "大右转", "急右转"]
            },
            "tree": {
                "depth": max_depth,
                "accuracy": float(accuracy),
                "tree": tree_to_dict(tree.tree_, self.feature_names)
            }
        }
    ...
    def generate_if_rules(self, output_path="if_rules.cs", max_depth=8, language="csharp",
        ...
        Args:
            ...
            optimize_w: 已废弃（保留兼容性）
            w_depth_boost: 已废弃（保留兼容性）
            auto_optimize: 是否自动优化参数以达到目标准确率（默认False）
            target_accuracy: 目标准确率（默认0.95）
            max_code_lines: 最大代码行数限制
            warn_on_large: 是否警告大型代码文件
        """
        # 自动优化模式：寻找最优参数
        if auto_optimize:
            print("\n" + "="*60)
            ...
            # 使用找到的最优参数
            depth = optimal_params['params']['max_depth'] or 50
            min_samples_leaf = optimal_params['params']['min_samples_leaf']
            min_samples_split = optimal_params['params']['min_samples_split']
            
            print(f"\n使用优化后的参数:")
            print(f"  depth={depth}, min_leaf={min_samples_leaf}, min_split={min_samples_split}, 准确率={optimal_params['accuracy']:.4f}")
        else:
            # 手动参数模式
            depth = max_depth
        ...
        print(f"\n[训练决策树]")
        print(f"  depth={depth}, min_leaf={min_samples_leaf}, min_split={min_samples_split}")
        ...
        # 估算代码行数（每个节点大约4-5行代码）
        estimated_lines = tree.tree_.node_count * 4
        estimated_total_lines = estimated_lines + 50  # +50 for class structure
        
        print(f"\n[代码大小估算]")
        print(f"  节点数: {tree.tree_.node_count}, 估算代码行数: ~{estimated_lines}")
        ...
        if estimated_total_lines > max_code_lines:
            ...
            print(f"     2. 降低目标准确率（如 --target-accuracy 0.90）")
        ...
        # 打印分类报告
        print(f"\n[分类报告]")
        report = classification_report(self.y, pred, 
                                       target_names=["急左转", "大左转", "左转", "小左转", "微左转", "直行", 
                                                    "微右转", "小右转", "右转", "大右转", "急右转"],
                                       zero_division=0)
        print(report)
...
{{
    ...
    {{
{tree_to_code(tree.tree_, self.feature_names, base_indent=2, language="csharp")}
    }}
}}
"""
        else:  # python
...
def predict_discrete_action(sensor0, sensor1, sensor2, sensor3, sensor4, sensor5,
                            last_action_state,
                            actual_vz, actual_vx, actual_omega):
{tree_to_code(tree.tree_, self.feature_names, base_indent=1, language="python")}
"""
        ...
        if actual_lines > max_code_lines:
            ...
            print(f"        python analysis.py --data <file> --rules --depth 15 --min-leaf 3 --target-accuracy 0.90")
...
def main():
    ...
    parser.add_argument('--target-r2', type=float, default=None, help='自动优化模式下的目标准确率（会覆盖全局变量，兼容旧参数名）')
    parser.add_argument('--target-accuracy', type=float, default=None, help='自动优化模式下的目标准确率（会覆盖全局变量）')
    ...
    # 处理目标准确率：优先使用target_accuracy，其次target_r2（兼容旧参数名），最后使用全局变量
    if args.target_accuracy is not None:
        target_accuracy = args.target_accuracy
    elif args.target_r2 is not None:
        target_accuracy = args.target_r2
    elif hasattr(globals(), 'TARGET_ACCURACY') and TARGET_ACCURACY is not None:
        target_accuracy = TARGET_ACCURACY
    elif TARGET_R2_X is not None:
        target_accuracy = TARGET_R2_X  # 兼容旧配置
    else:
        target_accuracy = 0.95  # 默认值
